function [net,stats,OA,AA,Kappa,CA,accu_martix] = cnn_train_dag_HSI(net, imdb, getBatch, varargin)
...
for epoch=start+1:opts.numEpochs
  ...
  if numel(opts.gpus) <= 1
    ...
    [OA(epoch),AA(epoch),Kappa(epoch),CA(:,epoch),accu_martix(:,epoch)] = process_test(net,imdb.images.test_samples,imdb.images.test_labels);
%     disp(OA(epoch))
    if ~evaluateMode

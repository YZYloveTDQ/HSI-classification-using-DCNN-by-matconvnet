function [net,stats]  = cnn_train_acc(net, imdb, getBatch, varargin)
...
for epoch=start+1:opts.numEpochs
  ...
  if numel(params.gpus) <= 1
    [net, state,res] = processEpoch(net, state, params, 'train') ;
   %  [net, state,res] = processEpoch(net, state, params, 'val') ;
%     [OA(epoch),AA(epoch),Kappa(epoch)] = process_test(net,imdb.images.test_samples,imdb.images.num_scales,imdb.images.test_labels);
    if ~evaluateMode
...
% -------------------------------------------------------------------------
function [net, state,res] = processEpoch(net, state, params, mode)
% -------------------------------------------------------------------------
